...
const captcha = () => root.querySelector('[name=turnstileToken]')?.value || ''
const form = (title, fields, button, links = '') => `<h1 class="text-3xl font-black">${title}</h1><form class="mt-6 space-y-4">${fields}<input type="hidden" name="turnstileToken"><div data-turnstile></div><button class="w-full rounded-xl bg-slate-950 px-5 py-3 font-semibold text-white">${button}</button></form>${links}`
...
function renderCaptcha() { const key = window.APP_CONFIG?.TURNSTILE_SITE_KEY; if (key && window.turnstile) window.turnstile.render(root.querySelector('[data-turnstile]'), { sitekey: key, callback: token => { root.querySelector('[name=turnstileToken]').value = token } }) }
...
if (!['verified', 'sent'].includes(state)) {
  ...
  root.querySelector('form').addEventListener('submit', async event => { event.preventDefault(); const button = root.querySelector('button'); setBusy(button, true); const data = Object.fromEntries(new FormData(event.currentTarget)); try {
    const headers = { 'X-Captcha-Response': captcha() }
    if (state === 'signup') { await api('/api/auth/sign-up/email', { method: 'POST', headers, body: JSON.stringify({ ...data, callbackURL: '/auth.html?state=verified' }) }); go('sent') }
    else if (state === 'forgot') { await api('/api/auth/request-password-reset', { method: 'POST', headers, body: JSON.stringify({ email: data.email, redirectTo: '/auth.html?state=reset' }) }); root.innerHTML = '<h1 class="text-3xl font-black">Check your inbox</h1><p class="mt-3 text-slate-500">If that address has an account, a reset link is on its way.</p>' }
    ...
    else if (state === 'resend') { await api('/api/auth/send-verification-email', { method: 'POST', headers, body: JSON.stringify({ email: data.email, callbackURL: '/auth.html?state=verified' }) }); go('sent') }
    else { await api('/api/auth/sign-in/email', { method: 'POST', headers, body: JSON.stringify({ email: data.email, password: data.password }) }); location.href = next }
